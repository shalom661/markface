...
async def get_fixed_costs(db: AsyncSession = Depends(get_session)):
    return await finance_service.list_fixed_costs(db)

@router.post("/fixed-costs", response_model=FixedCostRead)
async def post_fixed_cost(schema: FixedCostCreate, db: AsyncSession = Depends(get_session)):
    return await finance_service.create_fixed_cost(db, schema)

@router.delete("/fixed-costs/{cost_id}")
async def remove_fixed_cost(cost_id: str, db: AsyncSession = Depends(get_session)):
...
async def post_purchase(schema: PurchaseCreate, db: AsyncSession = Depends(get_session)):
